...
- ~~🔒 **前台专注检测** - 检测前台是否为娱乐应用，配合惩罚机制使用（lwq负责）~~ ✅已完成
- 📈 **数据可视化** - 统计图表展示专注趋势
- ☁️ **云端同步** - 数据跨设备同步
- 🌙 **深色模式** - 支持明暗主题切换（ygd负责）
- ⏰ **定时提醒** - 每日固定时间提醒开始专注
...
- ✅ 4.1 拨杆功能完善：
  ...
  - [ ] 4.1.3 应用启动时恢复专注模式状态
- ✅ 4.2 计时器与作物成长关联：番茄钟运行时每分钟增加作物进度
- ✅ 4.3 实现重置惩罚机制：专注模式下重置番茄钟导致作物枯萎
- ✅ 4.4 专注模式拨杆在番茄钟运行时禁用
- ✅ 4.5 专注模式下番茄钟运行时禁止种植作物
- ✅ 4.6 专注模式下番茄钟暂停时不允许切换专注模式
- ✅ 4.7 自定义确认弹窗（红色主题）
...
- [ ] 5.1 商店系统：
  ...
  - [ ] 5.1.4 实现作物出售功能（金币获取）
- [ ] 5.2 作物背包系统：
  - [ ] 5.2.1 设计作物背包按钮（菜园子页面内）
  - [ ] 5.2.2 创建作物背包弹窗界面
  - [ ] 5.2.3 实现作物收获进入背包功能
  - [ ] 5.2.4 实现背包查看已收获作物功能
- [ ] 5.3 签到系统：
  - [ ] 5.3.1 设计签到按钮和签到弹窗
  - [ ] 5.3.2 实现每日签到获取基础种子功能
...
| 菜园子页面内 | 添加「商店」按钮，点击打开商店弹窗（待开发）|
| 菜园子页面内 | 添加「作物背包」按钮，点击查看已收获作物（待开发）|
| 菜园子页面内 | 添加「签到」按钮，每日签到获取种子（待开发）|
...
在文本框中用自然语言描述您的工作或学习需求，例如：

- "我要准备考试，需要学习3小时"
- "写代码2小时，每25分钟休息5分钟"
- "完成报告，工作45分钟休息10分钟，重复4次"
- "学习英语1小时，每学习20分钟休息5分钟"

###### 3. 生成计划

点击"✨ 生成计划"按钮，AI会分析您的需求并生成合理的番茄钟计划。
...
查看生成的计划，如果满意，点击"✅ 应用计划"按钮。

**注意**：
- 番茄钟运行时，"应用计划"按钮会被禁用
- 需要先停止当前番茄钟才能应用新计划

计划会自动添加到左侧的计划列表中，并切换到计划模式。

###### 5. 开始执行

点击"开始"按钮，番茄钟会按照AI生成的计划依次执行。
...
1. **API费用**：DeepSeek API 是付费服务，请注意您的账户余额
2. **网络连接**：需要稳定的网络连接才能使用AI助手
3. **响应时间**：AI生成计划通常需要2-5秒
4. **隐私保护**：您的输入会发送到DeepSeek服务器进行处理
5. **数据持久化**：API Key 保存在本地 `data.json` 文件中

##### 常见问题

**Q: 提示"请先配置 API Key"怎么办？**

A: 点击顶部的 🔑 按钮，在弹出的窗口中输入您的 DeepSeek API Key 并保存。

**Q: 提示"生成计划失败"怎么办？**

A: 请检查：
1. API Key 是否正确配置（点击 🔑 查看状态）
2. 网络连接是否正常
3. DeepSeek 账户是否有余额

**Q: 如何查看我的 API Key 是否配置成功？**

A: 点击顶部的 🔑 按钮，查看配置窗口：
- 如果显示"✅ 已配置"和遮蔽后的 Key，说明配置成功
- 如果显示"❌ 未配置"，需要重新配置

**Q: 可以修改AI的规划逻辑吗？**

A: 可以。打开 `src/modules/aiAssistant.js`，修改 `systemPrompt` 变量中的提示词。
...
1. 重新描述需求，更详细地说明您的要求
2. 生成后手动在计划列表中调整
3. 修改 AI 的系统提示词以改变规划风格

**Q: API Key 会被上传到云端吗？**

A: 不会。API Key 只保存在您本地的 `data.json` 文件中，不会上传到任何服务器。

**Q: 前台检测也需要配置 API Key 吗？**

A: 不需要单独配置。配置一次 API Key 后，AI规划助手和前台检测会自动共享使用。
...
- ✅ 4.4 「不是娱乐」按钮：根据来源将窗口移入白名单或标记历史记录

**第五阶段：完善与优化**
- ✅ 5.1 警告弹窗弹出时抢占前台（窗口置顶）
- ✅ 5.2 API Key 验证：启动时检查配置有效性
- ✅ 5.3 错误提示弹窗：API Key 无效时显示配置路径
- [ ] 5.4 History 列表智能归纳（待优化）
...
- ~~🎵 **音乐播放器** - 基础播放功能~~
- ~~🎧 **输出设备切换** - 支持切换音频输出设备~~
- ~~🔊 **音乐播放器音量滑块** - 点击音量按钮展开竖向滑块控制音量~~
- ~~⏮️ **上一首按钮禁用** - 无上一首时按钮变灰禁用~~
- ~~⏳ **启动加载页面** - 显示 Python 进程启动进度，提升启动体验~~
- ~~🐛 **音乐播放器 bug 修复** - 重构状态模型，修复暂停切歌死机问题~~
...
- ~~🔒 **前台专注检测（第三阶段）** - Electron 集成：进程通信、专注模式联动~~
- ~~🔒 **前台专注检测（第四阶段）** - 警告弹窗与惩罚：三次警告触发作物枯萎~~
...
│   │       └── theme.js            # 明暗切换
│   │       └── foregroundDetection.js  # 前台检测模块（渲染进程）
│   └── modules/
│       └── musicProcess.js     # 音乐进程管理
│       └── aiAssistant.js     # AI助手进程管理
│       └── foregroundInspection.js  # 前台检测进程管理
