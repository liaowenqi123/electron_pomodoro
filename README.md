...
**当前的 `music.exe` 是一个测试模拟程序**，仅用于验证 Electron 与 Python 的通信接口。它不会真正播放音乐，只是模拟播放状态。

待真正的音乐播放器开发完成后，替换 `music-player/music.exe` 即可。

### 自定义音乐

将音乐文件放入 `music-player/music/` 文件夹即可（需要真正的播放器支持）。
